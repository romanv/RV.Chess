...
namespace RV.Chess.Board
{
    ...
    public static class Zobrist
    {
        ...
        private static int HashCastling (CastlingRights cr)
        {
            var hash = 0;

            if (cr.Rights.HasFlag(CastlingDirection.WhiteKingside)) hash ^= POLYGLOT[768];
            if (cr.Rights.HasFlag(CastlingDirection.WhiteQueenside)) hash ^= POLYGLOT[768 + 1];
            if (cr.Rights.HasFlag(CastlingDirection.BlackKingside)) hash ^= POLYGLOT[768 + 2];
            if (cr.Rights.HasFlag(CastlingDirection.BlackQueenside)) hash ^= POLYGLOT[768 + 3];

            return hash;
        }

        private static int HashTurn(Chessgame game)
        {
            return game.SideToMove == Side.White ? POLYGLOT[780] : 0;
        }

        private static int HashEnPassant(Chessgame game)
        {
            return game.EnPassantSquareIdx > -1
                ? POLYGLOT[772 + game.EnPassantSquareIdx % 8]
                : 0;
        }
